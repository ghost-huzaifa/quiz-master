...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/quizzes', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const quizData = insertQuizSchema.parse({ ...req.body, createdBy: userId });
  ...
  app.post('/api/quizzes/:quizId/attempts', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const existingAttempt = await storage.getAttemptByStudentAndQuiz(userId, req.params.quizId);
      if (existingAttempt) {
  ...
  app.get('/api/quizzes/:quizId/attempts', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (user?.role !== 'teacher') {
        return res.status(403).json({ message: 'Only teachers can view quiz attempts' });
      }
  ...
  app.get('/api/students/:studentId/attempts', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      ...
      const attempts = await storage.getAttemptsByStudentId(requestedStudentId);
      res.json(attempts);
    } catch (error) {
